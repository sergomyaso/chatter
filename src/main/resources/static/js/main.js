...
function getListMessages() {
    return httpGet("/listMessages");
}

function parseMessages(listMessages) {
    listMessages = JSON.parse(listMessages);
    for (var index = 0; index < listMessages.length; ++index) {
        parseMessage(listMessages[index])
    }
}

function showOldMessages() {
    parseMessages(getListMessages());
